...
int ofxfs2_nvglCreateImageFromHandleGL2(NVGcontext* ctx, GLuint textureId, int w, int h, int flags);
GLuint ofxfs2_nvglImageHandleGL2(NVGcontext* ctx, int image);

#endif

#if defined NANOVG_GL3

NVGcontext* ofxfs2_nvgCreateGL23(int flags);
void ofxfs2_nvgDeleteGL3(NVGcontext* ctx);

int ofxfs2_nvglCreateImageFromHandleGL3(NVGcontext* ctx, GLuint textureId, int w, int h, int flags);
GLuint ofxfs2_nvglImageHandleGL3(NVGcontext* ctx, int image);

#endif

#if defined NANOVG_GLES2

NVGcontext* ofxfs2_nvgCreateGL2ES2(int flags);
void ofxfs2_nvgDeleteGLES2(NVGcontext* ctx);

int ofxfs2_nvglCreateImageFromHandleGLES2(NVGcontext* ctx, GLuint textureId, int w, int h, int flags);
GLuint ofxfs2_nvglImageHandleGLES2(NVGcontext* ctx, int image);

#endif

#if defined NANOVG_GLES3

NVGcontext* ofxfs2_nvgCreateGL2ES3(int flags);
void ofxfs2_nvgDeleteGLES3(NVGcontext* ctx);

int ofxfs2_nvglCreateImageFromHandleGLES3(NVGcontext* ctx, GLuint textureId, int w, int h, int flags);
GLuint ofxfs2_nvglImageHandleGLES3(NVGcontext* ctx, int image);
							   ...
							   NVGscissor* scissor, float width, float fringe, float strokeThr)
{
	...
	if (scissor->extent[0] < -0.5f || scissor->extent[1] < -0.5f) {
		...
		ofxfs2_nvgTransformInverse(invxform, scissor->xform);
	...
	if (paint->image != 0) {
		...
		if ((tex->flags & NVG_IMAGE_FLIPY) != 0) {
			float m1[6], m2[6];
			ofxfs2_nvgTransformTranslate(m1, 0.0f, frag->extent[1] * 0.5f);
			ofxfs2_nvgTransformMultiply(m1, paint->xform);
			ofxfs2_nvgTransformScale(m2, 1.0f, -1.0f);
			ofxfs2_nvgTransformMultiply(m2, m1);
			ofxfs2_nvgTransformTranslate(m1, 0.0f, -frag->extent[1] * 0.5f);
			ofxfs2_nvgTransformMultiply(m1, m2);
			ofxfs2_nvgTransformInverse(invxform, m1);
		} else {
			ofxfs2_nvgTransformInverse(invxform, paint->xform);
		}
...
static GLNVGfragUniforms* ofxfs2_nvg__fragUniformPtr(GLNVGcontext* gl, int i);
...
#else
	GLNVGfragUniforms* frag = ofxfs2_nvg__fragUniformPtr(gl, uniformOffset);
...
//		glnvg__convertPaint(gl, ofxfs2_nvg__fragUniformPtr(gl, call->uniformOffset + gl->fragSize), paint, scissor, strokeWidth, fringe, 1.0f - 0.5f/255.0f);
							  ...
							  const float* bounds, const NVGpath* paths, int npaths)
{
	...
	if (call->type == GLNVG_FILL) {
		...
		frag = ofxfs2_nvg__fragUniformPtr(gl, call->uniformOffset);
		memset(frag, 0, sizeof(*frag));
		frag->strokeThr = -1.0f;
		frag->type = NSVG_SHADER_SIMPLE;
		// Fill shader
		glnvg__convertPaint(gl, ofxfs2_nvg__fragUniformPtr(gl, call->uniformOffset + gl->fragSize), paint, scissor, fringe, fringe, -1.0f);
	} else {
		call->uniformOffset = glnvg__allocFragUniforms(gl, 1);
		if (call->uniformOffset == -1) goto error;
		// Fill shader
		glnvg__convertPaint(gl, ofxfs2_nvg__fragUniformPtr(gl, call->uniformOffset), paint, scissor, fringe, fringe, -1.0f);
	}
								...
								float strokeWidth, const NVGpath* paths, int npaths)
{
	...
	if (gl->flags & NVG_STENCIL_STROKES) {
		...
		glnvg__convertPaint(gl, ofxfs2_nvg__fragUniformPtr(gl, call->uniformOffset), paint, scissor, strokeWidth, fringe, -1.0f);
		glnvg__convertPaint(gl, ofxfs2_nvg__fragUniformPtr(gl, call->uniformOffset + gl->fragSize), paint, scissor, strokeWidth, fringe, 1.0f - 0.5f/255.0f);
...
#endif
{
	...
	ctx = ofxfs2_nvgCreateInternal(&params);
	if (ctx == NULL) goto error;

	return ctx;

error:
	// 'gl' is freed by ofxfs2_nvgDeleteInternal.
	if (ctx != NULL) ofxfs2_nvgDeleteInternal(ctx);
	return NULL;
}
...
#if defined NANOVG_GL2
int ofxfs2_nvglCreateImageFromHandleGL2(NVGcontext* ctx, GLuint textureId, int w, int h, int imageFlags)
#elif defined NANOVG_GL3
int ofxfs2_nvglCreateImageFromHandleGL3(NVGcontext* ctx, GLuint textureId, int w, int h, int imageFlags)
#elif defined NANOVG_GLES2
int ofxfs2_nvglCreateImageFromHandleGLES2(NVGcontext* ctx, GLuint textureId, int w, int h, int imageFlags)
#elif defined NANOVG_GLES3
int ofxfs2_nvglCreateImageFromHandleGLES3(NVGcontext* ctx, GLuint textureId, int w, int h, int imageFlags)
#endif
{
	GLNVGcontext* gl = (GLNVGcontext*)ofxfs2_nvgInternalParams(ctx)->userPtr;
	GLNVGtexture* tex = glnvg__allocTexture(gl);
